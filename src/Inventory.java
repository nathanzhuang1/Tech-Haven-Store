...
public class Inventory extends JFrame implements ActionListener {
	...
	public Inventory() {
		...
		//A2.1.1 - Setting the preferred dimensions of PANEL (computerPanelImage) to edit the previous size
		computerPanelImage.setPreferredSize(new Dimension(1400, 3500)); //(width, height) 
		...
		switchButton.setOpaque(true); //ensures that the button background is transparent
		switchButton.setFocusPainted(false); //erases user selection of the button
		switchButton.setBorderPainted(false); //ensures that the button is colored accordingly
	...
	private void createLaptopPanels() {
		...
		for(int index = 0; index < laptopPanelArray.length; index++) {
			...
			laptopPanelArray[index].addMouseListener(new MouseAdapter() {
	            @Override
	            public void mouseClicked(MouseEvent e) {
	                // Action to perform when the panel is clicked
	                JOptionPane.showMessageDialog(null, "Panel clicked!");
	            }
	        });
			...
			laptopImageArray[index].setHorizontalAlignment(SwingConstants.CENTER); //this centers the image
			
			//'A1.3 - This calls the application method via fileInput to obtain the BRAND of each picture
			laptopTextArray[index] = new JLabel(TechHavenApplication.laptopArray[index].getBrand() + 
					" " + TechHavenApplication.laptopArray[index].getModel());
			
			//'A1.3.1 - This sets the font of each text (containing the brand and the model
			laptopTextArray[index].setFont(new Font("Hack", Font.BOLD, 14));
			
			//'A1.3.2 - This centers each text 
